      ...
      if mcontroller.onGround() then
        ...
        if self.voreSlam then
          ...
          for _, preyId in pairs(entities) do
            if starPounds.moduleFunc("pred", "eat", preyId, {ignoreCapacity = true, ignoreEnergyRequirement = true, energyMultiplier = 0.5, noSwallowSound = true, noSwallowBelch = true, playSquelchSound = true, loud = true, triggerPreyCooldown = true}) then
              hasPrey = true
              break
            end
          end
...
function doSlam()
  mcontroller.setYVelocity(-75)
  self.voreSlam = starPounds.isEnabled() and starPounds.moduleFunc("skills", "has", "voreSlam")
