...
function update(dt, fireMode, shiftHeld)
	...
	if storage.firing and animator.animationState("firing") == "off" then
		item.consume(1)
		if player then
			if (config.getParameter("content.type") == "pool") then
				local pool = config.getParameter("content.pool")
				local level = 0
				local seed = config.getParameter("content.seed")
				local treasure = root.createTreasure(pool, level, seed)
				for _,item in pairs(treasure) do
					player.giveItem(item)
				end
			else
				player.giveItem(config.getParameter("content.item"))
			end
		end
