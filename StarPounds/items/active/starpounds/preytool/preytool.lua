...
function activate(fireMode, shiftHeld)
  ...
  for _, target in ipairs(entities) do
    if isTargetValid(target) then
      world.sendEntityMessage(target, "starPounds.eatEntity", activeItem.ownerEntityId(), {ignoreSkills = true, ignoreCapacity = true, ignoreEnergyRequirement = true, energyMultiplier = 0})
      return
    end
  end
end
