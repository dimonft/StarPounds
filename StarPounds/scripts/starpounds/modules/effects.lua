    ...
    if effectData.duration then
      ...
      if effectData.duration == 0 then
        ...
        if effectConfig.expirePerLevel and (effectData.level > 1) then
          ...
          starPounds.events:fire("stats:calculate", "effects:levelDecrease")
        else
...
function effects:add(effect, duration, level)
  ...
  if effectConfig then
    ...
    if effectConfig.particle then
      ...
      world.spawnProjectile("invisibleprojectile", vec2.add(starPounds.mcontroller.position, starPounds.mcontroller.isNullColliding and 0 or vec2.div(starPounds.mcontroller.velocity, 60)), starPounds.entityId, {0,0}, true, {
        damageKind = "hidden",
    ...
    starPounds.events:fire("stats:calculate", "effects:add")
    return true
  end
  return false
end
...
function effects:remove(effect)
  ...
  if storage.starPounds.effects.active[effect] then
    ...
    starPounds.events:fire("stats:calculate", "effects:remove")
    if self.effects[effect] then
...
function effects.reset()
  ...
  starPounds.events:fire("stats:calculate", "effects:reset")
end
