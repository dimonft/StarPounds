...
function debug:update(dt)
  ...
  if starPounds.hasOption("showDebug") then
    ...
    local weightUnit = starPounds.settings.units[starPounds.getOption("weightUnit")]
    ...
    self:format("stomachContents", "<c:1>Contents: <c:3>%.1f <c:1>Food: <c:3>%.1f <c:1>Entities: <c:3>%d <c:1>Items: <c:3>%d", stomach.contents, stomach.food, #data.stomachEntities, #data.stomachItems)
    ...
    self:format("size", "<c:1>Size: <c:3>%s <c:1>Weight: <c:3>%.2f%s <c:1>Multiplier: <c:3>%.1fx", (starPounds.currentSize.size == "" and "none" or starPounds.currentSize.size)..(starPounds.currentVariant and ": "..starPounds.currentVariant or ""), data.weight * weightUnit[2], weightUnit[1], starPounds.weightMultiplier)
