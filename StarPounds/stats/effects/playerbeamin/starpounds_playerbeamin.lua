...
function update(dt)
  ...
  if not didQuery then
    ...
    for _, target in ipairs(entities) do
      promises:add(world.sendEntityMessage(target, "starPounds.eatEntity", entity.id(), {ignoreSkills = true, ignoreCapacity = true, ignoreEnergyRequirement = true, energyMultiplier = 0, noSwallowSound = true}), function(success)
